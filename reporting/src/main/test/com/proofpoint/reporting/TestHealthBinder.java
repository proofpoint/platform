    ...
    public void testHealth() {
        Injector injector = Guice.createInjector(
                ...
                new Module()
                {
                    ...
                    {
                        healthBinder(binder).export(HealthClass.class).withGeneratedName();
                    }
                });
    ...
    public void testHealthWithAnnotation() {
        Injector injector = Guice.createInjector(
                ...
                new Module()
                {
                    ...
                    public void configure(Binder binder)
                    {
                        healthBinder(binder).export(HealthClass.class).annotatedWith(TestingAnnotation.class).withGeneratedName();
                    }
                });
    ...
    public void testHealthWithNameAnnotation() {
        Injector injector = Guice.createInjector(
                ...
                new Module()
                {
                    ...
                    public void configure(Binder binder)
                    {
                        healthBinder(binder).export(HealthClass.class).annotatedWith(Names.named("TestingAnnotation")).withGeneratedName();
                    }
                });
        assertHealthRegistration(injector, ImmutableSet.of("Health check (TestingAnnotation)", "Not Bean Getter (TestingAnnotation)"));
    }
    ...
    public void testHealthWithName() {
        Injector injector = Guice.createInjector(
                ...
                new Module()
                {
                    ...
                    public void configure(Binder binder)
                    {
                        healthBinder(binder).export(HealthClass.class).as("Specified name");
                    }
                });
    ...
    public void testNested() {
        Injector injector = Guice.createInjector(
                ...
                new Module()
                {
                    ...
                    public void configure(Binder binder)
                    {
                        healthBinder(binder).export(NestedClass.class).withGeneratedName();
                    }
                });
    ...
    public void testFlatten() {
        Injector injector = Guice.createInjector(
                ...
                new Module()
                {
                    ...
                    public void configure(Binder binder)
                    {
                        healthBinder(binder).export(FlattenClass.class).withGeneratedName();
                    }
                });
        assertHealthRegistration(injector, ImmutableSet.of("Health check", "Not Bean Getter"));
    }

    private void assertHealthRegistration(Injector injector, Set<String> expectedAttributes)
    {
        HealthBeanRegistry registry = injector.getInstance(HealthBeanRegistry.class);

        Map<String, HealthBeanAttribute> healthBeans = registry.getHealthAttributes();

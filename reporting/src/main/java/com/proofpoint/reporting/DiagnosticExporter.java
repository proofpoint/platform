...
public class DiagnosticExporter
{
    ...
    {
        DiagnosticBean diagnosticBean = DiagnosticBean.forTarget(object);
        if (!diagnosticBean.getAttributes().isEmpty()) {
            try {
                registry.register(object, diagnosticBean, namePrefix);
            }
